...
| 选项 | 状态 |
|------|------|
| PostgreSQL | ✅ **已确认** — JSONB、并发强、开源协议宽松 |
| MySQL | ❌ 不使用 |
...
| # | 事项 | 决定 | 日期 |
|---|------|------|------|
| 1 | 数据库选型 | PostgreSQL | 2026-05-30 |
| 2 | 主键策略 | UUID v4（`gen_random_uuid()`） | 2026-05-30 |
| 3 | 迁移工具 | 待进入 Task-03 前确认 | — |
| 4 | 本地 SQLite 迁移机制 | 待进入 Task-03 前确认 | — |
| 5 | 是否可以开始执行数据库迁移？ | ❌ 尚未批准执行 | — |

---

> **状态**：数据库选型与主键策略已确认。迁移工具与 SQLite 机制待 Task-03 实现前确认。**尚未批准执行任何迁移。**
