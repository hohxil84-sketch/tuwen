...
每个模块完成并合并到 `main` 后，必须进入下一任务交接流程：

1. Codex 根据当前 Sprint 计划准备下一任务文档。
2. 下一任务文档必须包含：本次只开发什么、本次不开发什么、允许修改文件、禁止修改文件、验收标准、测试方式、是否允许新增依赖、是否涉及重大变更。
3. Codex 必须给出下一任务建议分支名。
4. Codex 必须保存或更新本次完成模块的上下文文件：`docs/module-context/<module-or-task>/context.md`。
5. 用户开启新会话，并在新会话中说明项目路径、目标分支、任务文档和模块上下文路径。
6. Claude Code / DeepSeek 只能在新任务分支上执行新任务。

禁止在已完成模块的原分支上继续开发下一个模块。
禁止在没有下一任务文档的情况下启动下一个模块。

## 模块上下文提交规则

模块上下文文件用于后续扩展和修改时快速恢复背景。

- 上下文路径：`docs/module-context/<module-or-task>/context.md`。
- 每个模块完成、PR 合并或重要修改后必须更新。
- 后续修改同一模块前必须先读取该上下文。
- 上下文更新必须和对应任务文档或模块修改一起进入 PR，不允许长期只停留在本地。
- 上下文不得保存真实密钥、Token、生产数据库连接串或用户隐私数据。
