...
- 当前分支是 `main`
- 需要删除文件、重命名目录或做大规模重构
- 需要安装到本机的依赖、SDK、CLI、运行时、模型、缓存、构建工具或其他外部工具，但未优先安装/配置到 `D:` 盘
...
- 不混合多个模块。
- 不自行创建下一任务实现。
- 新增本地依赖、SDK、CLI、运行时、模型、缓存或构建工具时，默认安装/配置到 `D:` 盘；交付说明必须写明实际安装位置和缓存位置。
- 不得默认把新工具、依赖缓存、模型缓存或大体积构建缓存安装到 `C:` 盘；如无法避免，必须先暂停并等待用户确认。
