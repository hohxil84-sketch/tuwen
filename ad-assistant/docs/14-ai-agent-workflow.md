...
- 凡是需要安装到本机的桌面级应用，CC 默认必须安装到 `D:\APPLICATION`；凡是需要安装到本机的环境依赖、SDK、CLI、运行时、模型、缓存、构建工具或其他外部工具，默认必须安装或配置到 `D:\locaPath`；如果工具强制写入 `C:` 盘，必须先暂停说明原因、占用空间、风险和清理方式，等待用户确认。
