...
| # | 事项 | 决定 |
|---|------|------|
| 1 | 数据库选型 | **PostgreSQL** — JSONB、并发、迁移生态更优 |
| 2 | 主键策略 | **UUID v4** — `gen_random_uuid()`，简单可靠 |
| 3 | 依赖批准 | **PyJWT**（替代 python-jose）+ **passlib[bcrypt]** 批准 |
| 4 | 设备数量上限 | **3 台**，超限返回 `DEVICE_LIMIT_REACHED` |
| 5 | refresh token 有效期 | **30 天**，启用 rotation，云端只存 hash |
| 6 | access token 有效期 | **30 分钟**，客户端只存内存不落盘 |

以上事项已确认，可进入 Task-03 实现。
